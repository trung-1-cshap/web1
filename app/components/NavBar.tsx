    ...
    <header className="relative h-36 md:h-28 lg:h-40 header-gradient">
      ...
      <div className="relative z-10 max-w-7xl mx-auto flex items-center justify-between px-4 h-full">
        <Link href="/" className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-white">Quản lý thu chi</span>
        </Link>
        {user && (
          <nav className="hidden md:block flex-1">
            <ul className="flex justify-center gap-6">
              {navItems.map((it) => (
                <li key={it.href}>
                  <Link href={it.href} className={`px-4 py-2 rounded-md font-semibold transition ${isActive(it.href) ? "bg-white/30 text-white" : "text-white hover:text-[#48a0f7] hover:bg-white/10"}`}>{it.label}</Link>
                </li>
              ))}
            </ul>
          </nav>
        ...
        <div className="flex items-center gap-4">
          {user ? (
            <>
              <Link href="/profile" className="text-sm text-white hover:underline hidden sm:block">
                <div>Xin chào, {user.name}</div>
                <div className="text-xs text-white/70">{String(user.role ?? "user")}</div>
              </Link>
              <button onClick={logout} className="px-3 py-1 rounded-md bg-red-600 text-white text-sm">Đăng xuất</button>
            </>
          ) : (
            <Link href="/login" className="px-4 py-2 rounded-md font-bold text-white bg-gradient-to-r from-amber-400 to-amber-800">Đăng Nhập</Link>
          )}
        </div>
      </div>
    </header>
